...
modelSelection <- function(pred, ## verjetnostne napovedi (matrika)
							...
							uporabaModelov = c('accu', 'p/rF', 'rmse'), ## katere modele naj uporabi
											## accu: accuracy
	...
	if ('p/rF' %in% uporabaModelov){
		...
		tocke[F[1:n]] = tocke[F[1:n]] + utezKriterija[2]*nSk
	}
	## root mean squared error
	if ('rmse' %in% uporabaModelov){
		errors <- sqrt(colSums(abs(pred - Y)))
		rse <- order(errors)
		tocke[rse[1:n]] = tocke[rse[1:n]] + utezKriterija[3]*nSk
	}
...
# t1 <- Sys.time()
# rez <- modelSelection(X,Y, n = 2, rev(1:2), uporabaModelov = c('accu'), utezKriterija = c(2,1,1))
# t2 <- Sys.time()
# t2-t1
